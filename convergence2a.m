...
for j=1:21
    ...
    for k=1:length(tnext)
        temp=(abs((M2(k)-Mnext(k,2)')/M2(k)))...
                          +(abs((M3(k)-Mnext(k,3)')/M3(k)));
        if temp<0.5
            change2(j)=change2(j)+temp/2; %Skip anomalously large values, cause of division by zero
        else 
            count=count+1;
        end
    end
        change2(j)=change2(j)/(length(tnext)-count); %Average percentage change per time point
        print('done')
end  

figure; plot(linspace(100,400,21),change2,'o--');

figure; 
plot(linspace(100,400,21),change,'x-r','MarkerSize',8,'LineWidth',1);
hold on; plot(linspace(100,400,21),change2,'o-b','LineWidth',1);
set(gca,'FontSize',14);
